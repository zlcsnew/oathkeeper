...
func (m *RepositoryMemory) Append(ctx context.Context, rules []Rule) error {
	if len(rules) == 0 {
		return errors.WithStack(helper.ErrMatchesNoRule)
	}
	m.Lock()
	defer m.Unlock()
...
func (m *RepositoryMemory) Delete(ctx context.Context, rule_id string) error {
	m.Lock()
	defer m.Unlock()
...
func (m *RepositoryMemory) Change(ctx context.Context, rule_id string, rule_after_change Rule) error {
	m.Lock()
	defer m.Unlock()
	...
	m.rules[index] = rule_after_change
	return nil
}
